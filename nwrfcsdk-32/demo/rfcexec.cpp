...
RFC_RC RfcExecServer::handleRequest(RFC_CONNECTION_HANDLE rfcHandle, RFC_FUNCTION_HANDLE funcHandle, RFC_ERROR_INFO *errorInfo){
	...
	if (*readPipe == cU('X')){
		...
		if (rc != RFC_OK){
			trace(cU("Writing PIPEDATA table failed"), errorInfo->message, 1);
			return RFC_EXTERNAL_FAILURE;
		}
		while(fgetsU(command, 80, pipe)){
			line = RfcAppendNewRow(pipedata, errorInfo);
			if (line == NULL) return RFC_EXTERNAL_FAILURE;
			RfcSetChars(line, cU("PIPEDATA"), command, strlenU(command), errorInfo);
		}
	}
  #ifdef SAPwithPASE400
    // for RFCEXEC we only support the standard UNIX SAP Implementation 
	// pclose and not the platform dependent pclose -> as4_pclose.
